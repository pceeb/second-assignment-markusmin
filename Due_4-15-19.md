...
To log into Hoffman2, you need to enter the following code:

ssh your_user_name@hoffman2.idre.ucla.edu
When prompted, enter your password.

2. What is the first thing that you do once you login?  What is the command?

Once logged in, you should get a compute node by putting "qrsh" in the command line and hitting enter.
...
ls -l causes the ls command to list items in your folder in a long listing format. It includes more information about your folders, such as the date when the folder was last modified and how many bytes it is.

If you use both the -l and -h flags, the file sizes are then listed in "human readable format," meaning they are printed in terms of MB or KB instead of just raw bytes.
...
The command "ls -R -t" lists the contents of all directories recursively, with the contents of each directory sorted by time of last change (with the most recently edited files at the top).
...
1. what is ~
"~" is a shortcut for home - e.g. "cd ~" gets you back to your home directory, and "ls ~" lists the directory contents of your home directory.

2. what is ../
"../" is a shortcut for the previous directory that your current directory is nested within.
...
````
$ cd .
- "." just stands for current directory so "cd ." just goes into your current directory, which means you don't change your directory.
$ cd /
- "cd /" takes you back to the root directory of your current directory. This unveils a whole bunch of stuff that's normally hidden.
$ cd /home/amanda
- this command wouldn't work because there is no directory called "home"
$ cd ../..
- This would take you back two directories all the ways back to /Users/.
$ cd ~
- This would take you all the way back the /Users/ directory.
$ cd home
- There is no "home" directory so this wouldn't do anything.
$ cd ~/data/..
- This wouldn't work, because you'd be telling terminal to look for a directory called "data" in the /Users/ directory, and this doesn't exist.
$ cd
- This would take you back to your home directory, which in this case is /Users/amanda, and thus would work!
$ cd ..
- This would take you back one directory, and thus would also work!
````
---
### File systems
...
It creates a text file called "my_file.txt". Yes, it shows up in my current directory.
...
It's a quick way to create a completely blank text file without any formatting whatsoever.

 Can you use touch to edit my_file.txt?

No - it just overwrites the file that you've already created by creating a new blank file with the same name.
...
">" - single redirect will just overwrite your current text file. That means if you run "echo hello > testfile01.txt" twice, you will just get one "hello" in your text file.
">>" - a double redirect will not overwrite your current text file, and instead adds to it. That means that if you run "echo hello >testfile02.txt" twice, you will get two "hello"s in your text file.
...
Using the flag "-i" after the "rm" command prompts you before removing it to check and make sure that you actually want to remove the file.
...
It copies both files into the directory!
...
It gives you an error because you didn't specify a directory to copy your files to!
...
She should run the following code:
```
...
These commands would only work if you had first created the following file structure:
2016-05-20/
└── data
$ mkdir 2016-05-20/data/raw
$ mkdir 2016-05-20/data/processed

These commands would not create the file structure you're looking for. Instead, they would create the following file structure:
2016-05-20/
└── data
└── raw
└── processed
...
$ mkdir data
$ mkdir raw processed
- if you added the command "$ cd data" before "$ mkdir raw processed" you would achieve the desired file structure.
```
---
...
Because the flag "-n" means numeric sort. Otherwise, the default is alphabetical sorting, leading to all numbers beginning with 1 being placed before all numbers beginning with 2.
...
A possible reason for uniq only removing adjacent duplicated lines is to only remove accidentally duplicated lines, rather than identical lines that show up in different parts of the dataset because they are different values. You could combine uniq with sort in order to remove all duplicated lines.
...
You could write the following text:
```
cut -d , -f 2 animals.txt | sort > animals_unique.txt | uniq animals_unique.txt
```
...
The first expression using [ ] syntax would provide them all together as one list, while my expression would provide two separate lists.
